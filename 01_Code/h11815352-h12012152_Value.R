...
Val_ret <- xts(Data$HML, Date)
Val_ret <- Val_ret / 100 ## To have the returns in decimals, not in percent.

#==== 02b - Source the Input Data (Global Factors) ============================#

# Path <- file.path(Data_Path, "[usa]_[value]_[daily]_[vw_cap].csv")
# Path <- file.path(Data_Path, "[usa]_[seasonality]_[daily]_[vw_cap].csv")
Path <- file.path(Data_Path, "[usa]_[short_term_reversal]_[daily]_[vw_cap].csv")

Data <- read.csv(Path) # Loads the Global Factor Data.

Date <- as.Date(Data$date)
Value_ret <- xts(as.numeric(Data$ret), Date)
...
Factor_ret_extension <- Value_ret[Extension_period] ## Factor return.
...
PM_names <- c("Mkt_rf", "Value", "Mkt_rf_managed", "Value_managed")
...
reg_fac_managed <- lm(Value_managed ~ Mkt_rf, 
                      data = Data_Analysis, 
                      na.action = na.omit)
...
reg_fac_on_factor <- lm(Value_managed ~ Value, 
                        data = Data_Analysis, 
                        na.action = na.omit)
...
stargazer(
  ...
  header = FALSE,
  title = "",
  column.labels = c("MktRF VM", "HML", "HML VM", "HML VM"),
  covariate.labels = c("MktRF", "Value", "Alpha"),
